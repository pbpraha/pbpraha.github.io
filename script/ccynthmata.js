...
class Ccynthmata {
    constructor(options){
        ...
        this._parameterDisplayElement = options.parameterDisplayElement
            ? options.parameterDisplayElement
            : this._interfaceRoot.querySelector(DEFAULT_PARAMETER_DISPLAY_SELECTOR);
        this._autoHideParameterDisplay = options.autoHideParameterDisplay || false;
        this._parameterDisplayTransforms = options.parameterDisplayTransforms
            ? options.parameterDisplayTransforms
            : {};
    ...
    *getCcElements(){
    ...
    tryAttachParameterChangeHandler(ccControl){
        ...
        ccControl.addEventListener("mouseover", ev => this._doParameterDisplay(ev));
        if("oninput" in ccControl){
            //ccControl.oninput = ev => this._handleValueChange(ev);
            ccControl.addEventListener("input", ev => this._handleValueChange(ev));
            ccControl.addEventListener("input", ev => this._doParameterDisplay(ev));
            return true;
        }else if("onchange" in ccControl){
            //ccControl.onchange = ev => this._handleValueChange(ev);
            ccControl.addEventListener("change", ev => this._handleValueChange(ev));
            ccControl.addEventListener("change", ev => this._doParameterDisplay(ev));
            return true;
        }
        return false;
    }
    ...
    _doParameterDisplay(event){
        if(!this._parameterDisplayElement){
            return; // go away
        }
        let disFunc = this._parameterDisplayTransforms[event.target.dataset.displayvaluefunc];
        let ccElementDetails = this.getCcElementDetails(event.target);
        if(disFunc){
            this._parameterDisplayElement.innerText = disFunc(ccElementDetails.value);
        }else{
            this._parameterDisplayElement.innerText = parseInt(ccElementDetails.value);
        }
        if(this._autoHideParameterDisplay){
            this._parameterDisplayElement.style.display = "block";
            if (this._valueDisplayTimeout != null) {
                clearTimeout(this._valueDisplayTimeout);
            }
            this._valueDisplayTimeout = setTimeout(() => {
                this._parameterDisplayElement.style.display = "none";
            }, VALUE_DISPLAY_TIMEOUT_MS);
        }
    }
